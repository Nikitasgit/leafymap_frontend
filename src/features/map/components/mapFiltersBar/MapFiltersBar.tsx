    ...
    > => {
      if (searchType.key === "evenements") {
        ...
        return events.reduce<EventSearchResult[]>((results, event) => {
            ...
            const image = resolveRefObject(event.image)?.urls?.thumbnail;
            const eventCategoryName = resolveRefObject(
              event.eventCategory,
            )?.name;
            results.push({
              id: event.id,
      ...
      if (searchType.key === "membres") {
        ...
        return creators.map((user) => ({
          ...
          categories: user.userCategory
            ? [
                {
                  name: resolveRefObject(user.userCategory)?.name ?? "",
                  type: resolveRefObject(user.userCategory)?.type,
                },
              ]
            : [],
        }));
