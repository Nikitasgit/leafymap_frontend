...
import { getEventLocationLabel } from "@/features/events/utils/eventRefs";
...
const EventCard: React.FC<EventCardProps> = ({ event, place, user }) => {
  ...
  const eventPlace = resolveRefObject(event.place);
  const displayPlace = place ?? eventPlace;
  const eventUser = resolveRefObject(event.user);
  const displayUser = user ?? eventUser;
  const locationLabel = event.online
    ? t("eventCard.online")
    : getEventLocationLabel(event) || displayPlace?.location?.label;
  const eventCategory = resolveRefObject(event.eventCategory);
  const eventCategoryName = eventCategory
    ? t(`common:eventCategories.${eventCategory.name}`, {
