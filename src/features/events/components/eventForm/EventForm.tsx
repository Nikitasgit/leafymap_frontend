"use client";
import { FormDataChangeHandler } from "@/features/account";
...
import type {
  Partnership,
  PartnershipPopulated,
} from "@/features/partnerships/types";
import { useSubmitEventInvitations } from "@/features/eventInvitations/hooks/useSubmitEventInvitations";
import { separateNewAndUpdatedArrayValues } from "@/shared/utils/tempId";
...
import PartnershipsForm from "@/features/partnerships/components/partnershipsForm";
import { useCurrentUser } from "@/features/auth";
import LocationPicker from "@/features/map/components/locationPicker";
import type { Event } from "../../types/event";
import { EventCategorySelectorInput } from "@/features/account";
...
const initialEventData = (
  ...
  eventCategory: resolveRefId(event?.eventCategory) ?? "",
  place: resolveRefId(event?.place),
...
const EventForm = ({
  eventData = null,
  isUpdate = false,
  partnershipsData = EMPTY_PARTNERSHIPS,
}: EventFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (event) {
        ...
        const submittedEvent = await submitEvent(
          eventPayload,
          isUpdate,
          eventData?.id
        );
        const eventId = submittedEvent?.id;
